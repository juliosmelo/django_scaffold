...
class Command(BaseCommand):
    ...
    def create_model(self, app_path, klass, attrs):
        with open(os.path.join(app_path, 'models.py'), 'w') as model_file:
            ...
            model_file.write(PEP8_INDENT+'created_at=models.DataTimeField(auto_add_now=True)\n')
            for attr in attrs:
                field_name = attr.split(':')[0]
                field_type = attr.split(':')[1]
    ...
    def create_views(self, app_path, klass):
        with open(os.path.join(app_path, 'views.py'), 'w') as view_file:
            ...
            view_file.write('from django.view.generic.detail import DetailView\n')
            view_file.write('from django.view.generic.list import ListView\n')
            view_file.write('from django.view.generic.edit import CreateView, UpdateView, DeleteView\n\n')
    ...
    def create_urls(self, app_path, app_name, klass):
        with open(os.path.join(app_path, 'urls.py'), 'w') as urls_file:
            urls_file.write('from django.conf.url import patterns, url\n')
            urls_file.write('from .views import * \n\n')
            urls_file.write("urlspatterns = patterns('', \n")
